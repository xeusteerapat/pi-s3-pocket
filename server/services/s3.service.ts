...
export class S3Service {
    ...
    const objects = await Promise.all(
      (result.Contents ?? [])
        ...
        .map(async (item) => {
          ...
          try {
            const head = await this.client.send(new HeadObjectCommand({ Bucket: bucket, Key: item.Key! }));
  ...
  async createFolder(bucket: string, key: string) {
    ...
    await this.client.send(new PutObjectCommand({ Bucket: bucket, Key: folderKey, Body: Buffer.alloc(0) }));
  }
